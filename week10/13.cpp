...
bool f(pair<string, pair<string, double> > p1,  pair<string, pair<string, double> >  p2){
...
int main(){
    ...
    pair<string, pair<string, double> > a[n];

    for(int i = 0; i < n; ++i){
        cin >> sname >> gpa >> school;
        a[i] = make_pair(sname, make_pair(school, gpa));
    }
